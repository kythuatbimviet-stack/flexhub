    ...
    return (
        <div className="space-y-6 font-inter pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {isLoading ? (
                            Array.from({ length: 6 }).map((_, i) => (
                                <Card key={i} className="rounded-xl h-48 animate-pulse bg-gray-50 border-none shadow-sm" />
                            ))
                            ...
                            <div className="col-span-full py-20 text-center space-y-4">
                                ...
                                <p className="text-gray-400 font-medium lowercase">Chưa có giáo án nào được tạo.</p>
                                <Button variant="outline" onClick={() => setIsBuilderOpen(true)} className="rounded-xl px-8 h-11">Tạo ngay giáo án đầu tiên</Button>
                            </div>
                            ...
                            <Card key={template.id} className={cn(
                                ...
                                <div className="absolute top-3 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                                    <Checkbox
                                ...
                                <CardHeader className="pb-2 pl-8">
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="space-y-1">
                                            <Badge variant="outline" className="text-[10px] font-medium border-red-100 text-[#FD5771] bg-[#FD5771]/5">
                                                {template.level || 'Beginner'}
                                            </Badge>
                                            <CardTitle className="text-lg font-medium text-black dark:text-white group-hover:text-[#FD5771] transition-colors">
                                                {template.name}
                                            </CardTitle>
                                        </div>
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button variant="ghost" className="h-8 w-8 p-0 rounded-lg hover:bg-slate-50"><MoreHorizontal className="h-4 w-4" /></Button>
                                            </DropdownMenuTrigger>
                                            <DropdownMenuContent align="end" className="rounded-xl w-40">
                                                <DropdownMenuItem className="rounded-lg cursor-pointer"><Pencil className="mr-2 h-4 w-4 text-blue-500" /> Chỉnh sửa</DropdownMenuItem>
                                    ...
                                    <CardDescription className="text-xs line-clamp-2 min-h-[32px] text-black/60 dark:text-white/60">
                                        Mục tiêu: <span className="font-medium text-black dark:text-white">{template.goal || 'Tăng cường sức mạnh'}</span>
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="flex-1 pb-4">
                                    <div className="grid grid-cols-2 gap-3 mt-2">
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                            ...
                                            <span>{template.sessions?.length || 0} Buổi mẫu</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                ...
                                <div className="p-4 pt-0 mt-auto">
                                    <Button variant="secondary" className="w-full rounded-xl h-10 text-[13px] font-medium gap-2 hover:bg-[#FD5771]/5 hover:text-[#FD5771] border border-slate-100 dark:border-slate-800 transition-all opacity-0 group-hover:opacity-100">
                                        Chi tiết giáo án
                                        <ChevronRight className="w-4 h-4" />
                                    </Button>
                                    <div className="w-full text-center text-[11px] text-black/40 dark:text-white/40 py-2 group-hover:hidden transition-all font-normal tracking-tight">
                                        Xem chi tiết
                                    </div>
                                </div>
                            </Card>
            ...
            <Sheet open={isBuilderOpen} onOpenChange={setIsBuilderOpen}>
                <SheetContent 
                    side="right" 
                    className="w-full sm:max-w-[800px] p-0 flex flex-col font-inter overflow-hidden border-l border-slate-100 dark:border-slate-800 shadow-2xl"
                    resizable={true}
                    defaultWidth={760}
                    minWidth={320}
                    maxWidth={1200}
                >
                    <SheetHeader className="p-4 sm:p-6 pb-2 border-b bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 shrink-0">
                        <SheetTitle className="text-lg sm:text-2xl font-medium text-black dark:text-white flex items-center gap-2">
                            <Plus className="w-5 h-5 sm:w-6 sm:h-6 text-[#FD5771]" />
                            Thiết kế giáo án
                        </SheetTitle>
                        <SheetDescription className="hidden sm:block text-sm text-black/60 dark:text-white/60 font-normal">Xây dựng giáo án mẫu chuyên sâu cho PT.</SheetDescription>
                    </SheetHeader>
                    
                    <ScrollArea className="flex-1">
                        <div className="p-3 sm:p-6 pb-20 sm:pb-32">
                            <TrainingPlanBuilder 
                                onSuccess={() => {
