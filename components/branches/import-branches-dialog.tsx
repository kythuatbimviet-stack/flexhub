...
import { bulkCreateBranches } from '@/app/actions/branches'

export function ImportBranchesDialog({ onSuccess }: { onSuccess: () => void }) {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            reader.onload = async (event) => {
                ...
                if (result.success) {
                    toast.success(`Đã import thành công ${branchesToCreate.length} chi nhánh`)
                    setOpen(false)
                    setFile(null)
                    setPreviewData([])
                    onSuccess()
                ...
                <div className="p-8 border-b border-slate-100 dark:border-slate-900 bg-slate-50/50 dark:bg-slate-900/50">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
                            Tải lên tệp Excel chứa danh sách chi nhánh để thêm vào hệ thống.
                        </DialogDescription>
                    </DialogHeader>
                </div>
                                ...
                                <table className="w-full text-xs text-left">
                                    <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 border-b border-slate-100 dark:border-slate-800">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2">Người đại diện</th>
                                        </tr>
                                    </thead>
