...
export function ImportExcelExpenseDialog({ onSuccess }: ImportExcelExpenseDialogProps) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                const result = await bulkCreateExpense(expenseToInsert)

                if (!result.success) throw new Error(result.error)

                toast.success(`Đã nhập thành công ${expenseToInsert.length} khoản chi`)
                setOpen(false)
                onSuccess()
            } catch (error: any) {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-md rounded-3xl border-none shadow-2xl p-8 font-inter">
                <DialogHeader className="space-y-3">
                    <DialogTitle className="text-xl font-bold text-gray-900 leading-tight">Nhập khoản chi hàng loạt</DialogTitle>
                    <DialogDescription className="text-gray-500 text-sm">
                        Tải lên file Excel để ghi nhận nhiều khoản chi phí cùng lúc.
                    </DialogDescription>
                </DialogHeader>
                <div className="py-8">
                    <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-100 rounded-3xl p-12 bg-gray-50/30 hover:bg-rose-50/30 transition-all cursor-pointer group relative overflow-hidden">
                        <input
                            type="file"
                            accept=".xlsx, .xls"
                    ...
                    <div className="mt-8 p-5 bg-gray-50/50 rounded-2xl border border-gray-50 flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <Button
                                variant="link"
                                className="p-0 h-auto text-rose-600 text-xs font-bold mt-2 hover:no-underline"
