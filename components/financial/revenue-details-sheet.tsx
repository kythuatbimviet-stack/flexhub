...
export function RevenueDetailsSheet({ revenue, open, onOpenChange, onSuccess }: RevenueDetailsSheetProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const { financial_categories, branches: b, customers, ...updateData } = formData
    ...
    const InfoRow = ({ icon: Icon, label, value, name, type = "text" }: any) => (
        <div className="space-y-2">
            <Label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2">
                <Icon className="w-4 h-4 text-emerald-500/80 dark:text-emerald-400" />
                {label}
            </Label>
            {isEditing ? (
                type === 'textarea' ? (
                    <Textarea
                        ...
                        className="rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 min-h-[100px] text-sm focus:ring-2 focus:ring-emerald-500 shadow-sm border-2"
                    />
                ) : (
                    <Input
                        name={name}
                        ...
                        value={formData[name] ?? ''}
                        onChange={handleChange}
                        className="rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-11 text-sm focus:ring-2 focus:ring-emerald-500 shadow-sm border-2"
                    />
                )
            ) : (
                <p className="text-base font-medium text-slate-900 dark:text-slate-100 pl-6 border-l-2 border-emerald-50 dark:border-emerald-900/30 ml-2">
                    ...
                    <SheetHeader className="space-y-1">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-5">
                                <div className="w-16 h-16 rounded-[1.25rem] bg-emerald-600 flex items-center justify-center text-white shadow-2xl transition-transform hover:scale-105">
                                    <DollarSign className="w-10 h-10" />
                                </div>
                                <div>
                                    <SheetTitle className="text-2xl font-semibold text-slate-950 dark:text-white flex items-center gap-2">
                                        {isEditing ? 'Chỉnh sửa khoản thu' : `Khoản thu: ${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(revenue.amount)}`}
                                    </SheetTitle>
                                    <div className="flex items-center gap-3 mt-1.5">
                                        <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-2 py-0.5 rounded-md uppercase tracking-wider">
                                            {revenue.id.split('-')[0]}
                                        </span>
                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700" />
                                        <div className="flex items-center gap-1.5 px-3 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-widest bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
                ...
                <div className="flex-1 overflow-y-auto px-8 py-10 space-y-12">
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
                            ...
                            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Chi tiết giao dịch</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            <InfoRow icon={DollarSign} label="Số tiền (VND)" value={new Intl.NumberFormat('vi-VN').format(formData.amount)} name="amount" type="number" />
                            <div className="space-y-2">
                                <Label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2">
                                    <Tag className="w-4 h-4 text-emerald-500/80 dark:text-emerald-400" />
                                    Danh mục
                                </Label>
                                {isEditing ? (
                                    <Select onValueChange={(val) => setFormData((p: any) => ({ ...p, category_id: val }))} value={formData.category_id}>
                                        <SelectTrigger className="rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-11 text-sm focus:ring-2 focus:ring-emerald-500 shadow-sm border-2">
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2">
                                    <CreditCard className="w-4 h-4 text-emerald-500/80 dark:text-emerald-400" />
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
                            ...
                            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Phân bổ chi nhánh & Đối tượng</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            <div className="space-y-2">
                                <Label className="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-2">
                                    <Building2 className="w-4 h-4 text-emerald-500/80 dark:text-emerald-400" />
                                    Chi nhánh
                                </Label>
                                {isEditing ? (
                            ...
                            <InfoRow icon={User} label="Khách hàng" value={formData.customers?.name || '-'} name="customer_id" />
                        </div>
                ...
                <div className="p-8 bg-gray-50/30 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between gap-6">
                    {isEditing ? (
                        <>
                            ...
                            <Button
                                onClick={handleSave}
                                className="flex-1 rounded-xl h-12 font-bold text-xs uppercase tracking-widest bg-gray-950 dark:bg-emerald-600 text-white hover:bg-black dark:hover:bg-emerald-700 shadow-2xl shadow-gray-200 dark:shadow-emerald-900/20 transition-all active:scale-95"
                                disabled={loading}
                            >
